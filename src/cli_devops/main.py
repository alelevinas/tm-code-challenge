...
def _get_styled_status(status: commands.Status):
    return click.style(f"{status}", fg='green' if status is commands.Status.HEALTHY else 'red')
...
def health_check():
    try:
        ...
        print("This CLI can't continue without access to the cpx server data. Please check command options and try again")
        exit()


@click.group()
@click.option('--cp-url', default=DEFAULTS['CP_URL'], help='Base URL of the cloud providder')
@click.option('--cpu-threshold', default=DEFAULTS['CPU_THRESHOLD'], help='CPU threshold to determine an unhealthy service that could scale')
@click.option('--mem-threshold', default=DEFAULTS['MEM_THRESHOLD'], help='RAM Memory threshold to determine an unhealthy service that could scale')
...
def services():
    ...
    servers = [[s.ip, s.service, _get_styled_status(s.status),
                f"{s.cpu:.2f}%", f"{s.mem:.2f}%"] for s in servers]
...
def status(service):
    ...
    services = [[s.service, _get_styled_status(s.status), f"{s.avg_cpu:.2f}%",
                 f"{s.avg_mem:.2f}%"] for s in services_status]
...
time (until the command is stopped, e.g. ctrl + c)."""
    ...
    while True:
        ...
        servers = [[s.ip, s.service, _get_styled_status(
            s.status), f"{s.cpu}%", f"{s.mem}%"] for s in servers]
